...
public class DialogPnl extends JDialog {
	public DialogPnl(int lottoCount, JFrame mainPnl) {
		...
		JPanel includeNumChoicePanel = new JPanel(); // 번호 선택 패널과 자동 수동 반자동 버튼들 포함될 패널
...
//		JLabel winLabel = new JLabel(n + "등 당첨"); // 나중에 n을 바꿔줌(n을 count로 받아서)
		...
		buyLottoPanel.add(includeNumChoicePanel, borderLayout.CENTER);
		buyLottoPanel.add(includeSendButtonPanel, borderLayout.SOUTH);
		includeNumChoicePanel.add(numChoicePanel);
		includeNumChoicePanel.add(outoButton);
		includeNumChoicePanel.add(selfButton);
		includeNumChoicePanel.add(halfOutoButton);
